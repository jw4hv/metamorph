...
class RMILoss(nn.Module):
    ...
    def forward(self, input, target):
        ...
        # Apply sigmoid to get probabilities. See the final paragraph of section 4.
        if self.with_logits:
    ...
    def extract_region_vector(self, x):
        ...
        # Adjust kernel_size and stride based on spatial dimensions
        spatial_dimensions = x.size(-1)
        kernel_size = min(self.radius, spatial_dimensions)
        stride = min(stride, spatial_dimensions - kernel_size + 1)

        # Reshape x to have dimensions (batch_size, channels, product of spatial dimensions)
        x_reshaped = x.view(x.size(0), x.size(1), -1, spatial_dimensions)

        x_regions = F.unfold(x_reshaped, kernel_size=kernel_size, stride=stride)
        x_regions = x_regions.view(x.size(0), x.size(1), kernel_size ** 2, -1)
        return x_regions
    ...
    def downsample(self, x):
        ...
        if self.downsampling_method == 'max':
            return F.max_pool3d(x, kernel_size=self.stride, stride=self.stride, padding=padding)
        if self.downsampling_method == 'avg':
            return F.avg_pool3d(x, kernel_size=self.stride, stride=self.stride, padding=padding)
