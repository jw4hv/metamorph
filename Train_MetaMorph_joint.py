    ...
    for epoch in range(para.solver.epochs):
        ...
        for j, image_data in enumerate(trainloader):
            ...
            if (epoch <= para.model.pretrain_epoch):
                print ("Segmentation model warm up")
                src_pred = net_seg(src_bch)
                tar_pred = net_seg(tar_bch)
                dice_loss_1 = Dice().loss(src_pred, src_seg_bch)
                dice_loss_2 = Dice().loss(tar_pred, tar_seg_bch)
                loss_total = dice_loss_1 + dice_loss_2
            elif (epoch > para.model.pretrain_epoch):
                print ("Joint training")
                ''' Data Augmentation, include the augmented data as the partial training data'''
                # if  (para.model.augmentation == True) and (epoch > para.model.pretrain_epoch + 1):
                #     src_bch[1,...] = Idef_aug[1,...]
                #     src_seg_bch[1,...] = Idef_seg[1,...]
                src_pred = net_seg(src_bch)
                tar_pred = net_seg(tar_bch)
                dice_loss_1 = Dice().loss(src_pred, src_seg_bch)
                dice_loss_2 = Dice().loss(tar_pred, tar_seg_bch)
                dice_loss_total = dice_loss_1 + dice_loss_2
                union = torch.logical_or(src_pred,  tar_pred).float()
                binary_map = 1 - union
                ...
                if (flag == "FLDDMM"):  # Fourier LDDMM to perform geodesic shooting
                    ...
                    # if (para.model.augmentation == True):
                    #     Idef_aug = dfm
                    #     Idef_seg = Torchinterp(src_seg_bch, phiinv_bch)
                    ...
                    '''Generated augumented image and its label'''
                    # if (para.model.augmentation == True) and (j % 5 == 1):
                    #     Idef_aug = pred[0]
                    #     Idef_seg = pred[1]
                    if epoch <= para.model.pretrain_epoch:
                    ...
                    # if (para.model.augmentation == True) and (j % 5 == 1):
                    #     Idef_aug = Idef
                    #     Idef_seg = lm.interp(src_seg_bch, h)
